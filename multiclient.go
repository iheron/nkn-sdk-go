			...
			for {
				...
				case msg, ok := <-client.OnMessage.C:
					...
					if msg.Type == SessionType {
						...
						// Emit message receive event with subclient info
...
func (m *MultiClient) sendWithClient(clientID int, dests []string, payload *payloads.Payload, encrypted bool, maxHoldingSeconds int32) error {
	...
	if useCrossSend {
		...
		err := client.send(finalDests, payload, encrypted, maxHoldingSeconds)
		return err
	}

	// Normal send: add prefix only for this specific client
	finalDests := addMultiClientPrefix(dests, clientID)
	err := client.send(finalDests, payload, encrypted, maxHoldingSeconds)
	return err
}
...
func (m *MultiClient) Send(dests *nkngomobile.StringArray, data interface{}, config *MessageConfig) (*OnMessage, error) {
	...
	go func() {
		...
		// Deduplicate client IDs to avoid sending the same message twice from the same client
		seen := make(map[int]bool)
		uniqueClientIDs := make([]int, 0, len(clientIDs))
		for _, clientID := range clientIDs {
			if !seen[clientID] {
